...
export function lighten(hex: string, amount: number): string {
  ...
  const r = Math.min(
    255,
    Math.round(parseInt(value.slice(0, 2), 16) + 255 * amount),
  );
  const g = Math.min(
    255,
    Math.round(parseInt(value.slice(2, 4), 16) + 255 * amount),
  );
  const b = Math.min(
    255,
    Math.round(parseInt(value.slice(4, 6), 16) + 255 * amount),
  );
...
export function computeCentroid(coords: number[][]): {
  lat: number;
  lng: number;
} {
  const points =
...
export function getPolygonSlug(data: object | null) {
  ...
  return polygon?.properties?.slug && polygon.properties.slug !== '__draft__'
    ? polygon.properties.slug
    : null;
}
