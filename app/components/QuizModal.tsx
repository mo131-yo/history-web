      ...
      <div className="bg-[#0a0c14] border border-white/10 w-full max-w-xl rounded-[2.5rem] overflow-hidden shadow-[0_0_100px_rgba(197,160,89,0.1)]">
        <div className="px-8 py-6 border-b border-white/5 flex justify-between items-center bg-white/2">
          <div className="flex items-center gap-3 text-[#C5A059]">
            ...
            <h3 className="font-serif font-black uppercase tracking-[0.2em] text-xs">
              AI Түүхийн сорил • {year} он
            </h3>
          </div>
          <button
            onClick={onClose}
          ...
          {loading ? (
            <div className="flex flex-col items-center py-20 gap-6">
              ...
              <p className="text-white/40 text-[10px] font-black tracking-[0.3em] uppercase">
                AI түүхэн өгөгдлийг боловсруулж байна...
              </p>
            </div>
          ) : isFinished ? (
            <div className="text-center space-y-8 py-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-white/40 text-sm italic font-serif">
                  Таны түүхийн мэдлэг өндөр түвшинд байна.
                </p>
              </div>
              <div className="bg-white/3 rounded-3xl p-6 border border-white/5">
                <p className="text-[10px] text-white/30 uppercase font-bold tracking-[0.2em] mb-1">
                  Нийт оноо
                </p>
              ...
              <button
                onClick={onClose}
                className="w-full bg-[#C5A059] text-black py-5 rounded-2xl font-black uppercase tracking-[0.2em] text-xs hover:bg-[#d4ae6a] transition-all shadow-xl shadow-[#C5A059]/10"
              >
                Аялалаа үргэлжлүүлэх
              </button>
            </div>
          ) : questions.length > 0 ? (
                ...
                {questions[currentStep].options.map(
                  (opt: string, i: number) => (
                    <button
                      ...
                      className="group w-full py-5 px-8 rounded-2xl bg-white/2 border border-white/5 text-white/70 text-sm font-bold hover:bg-white/6 hover:border-[#C5A059]/40 transition-all text-left flex justify-between items-center active:scale-[0.98]"
                    >
                      <span className="flex items-center gap-4">
                        <span className="w-6 h-6 rounded-lg bg-white/5 flex items-center justify-center text-[10px] group-hover:bg-[#C5A059] group-hover:text-black transition-colors">
                          {i + 1}
                        </span>
                        {opt}
                      </span>
                      <ChevronRight
                        size={16}
                        className="opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all text-[#C5A059]"
                      />
                    </button>
                  ),
                )}
