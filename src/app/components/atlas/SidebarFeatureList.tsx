...
export function SidebarFeatureList({
  ...
  return (
    <div className="flex flex-1 flex-col gap-1.5 overflow-y-auto px-3 pb-2">
      {features.length === 0 && (
        <div
          className="px-4 py-8 mt-4 text-xs text-center rounded-lg"
      ...
      {features.map((feature) => {
        const selected =
          feature.properties.slug === selectedFeature?.properties.slug;
        ...
        return (
          <button
            ...
            className="relative w-full overflow-hidden text-left transition-all duration-150 rounded-lg group"
            style={{
              background: selected
          ...
          >
            <div
              className="absolute inset-0 transition-opacity duration-150 opacity-0 pointer-events-none group-hover:opacity-100"
              style={{ background: 'rgba(245,158,11,0.04)' }}
            />
            <div
              className="absolute bottom-2 left-0 top-2 w-0.75 rounded-full"
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <div
                  ...
                  style={{
                    ...
                    clipPath:
                      'polygon(20% 0%, 80% 0%, 100% 20%, 100% 75%, 50% 100%, 0% 75%, 0% 20%)',
                  }}
                />
              </div>
              <p
