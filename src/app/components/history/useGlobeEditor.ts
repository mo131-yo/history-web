...
>) {
  ...
  useEffect(() => {
    ...
    const handleKey = (event: KeyboardEvent) => {
      ...
      onDraftRingChange(
        ring.filter((_, index) => index !== selectedVertexRef.current),
      );
      onSelectVertex?.(null);
    };
...
export function updateGlobeDraftAtPoint(
  ...
  if (options.isCreatingRef.current)
    return options.onDraftRingChange(
      appendPointToDraftRing(options.draftRingRef.current, nextPoint),
    );
  if (
    options.addPointModeRef.current &&
    options.draftRingRef.current.length >= 4
  ) {
    return options.onDraftRingChange(
      insertPointIntoNearestSegment(options.draftRingRef.current, nextPoint),
    );
  }
}
