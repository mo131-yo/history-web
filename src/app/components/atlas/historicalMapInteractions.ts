...
// ) {
//   map.on("mousedown", "draft-vertices", (event: { preventDefault: () => void; originalEvent: { preventDefault: () => void; stopPropagation: () => void; }; features: { properties: { index: any; }; }[]; }) => {
...
//   map.on("dblclick", "draft-vertices", (event: { preventDefault: () => void; features: { properties: { index: any; }; }[]; }) => {
//     if (!isEditingRef.current) return;
//     event.preventDefault();
...
//   map.on("mousemove", (event: { lngLat: { lng: number; lat: number; }; }    ) => {
//     if (dragIndexRef.current === null) return;
//     const ring = [...draftRingRef.current];
...
import {
  appendPointToDraftRing,
  insertPointIntoNearestSegment,
} from "@/lib/geometry";
import {
  createDraftPolygon,
...
type DraftRing = Array<[number, number]>;

type LayerMouseEvent = MapMouseEvent & {
  features?: MapGeoJSONFeature[];
};

export function bindHistoricalWindowStop(
  map: MapLibreMap,
  dragIndexRef: MutableRefObject<number | null>
) {
  const stopDragging = () => {
...
export function bindHistoricalEditing(
  map: MapLibreMap,
  dragIndexRef: MutableRefObject<number | null>,
  draftRingRef: MutableRefObject<DraftRing>,
  isEditingRef: MutableRefObject<boolean>,
  isCreatingRef: MutableRefObject<boolean>,
  addPointModeRef: MutableRefObject<boolean>,
  onDraftRingChange: (ring: DraftRing) => void
) {
  map.on("mousedown", "draft-vertices", (event: LayerMouseEvent) => {
    if (!isEditingRef.current) return;
  ...
  map.on("dblclick", "draft-vertices", (event: LayerMouseEvent) => {
    if (!isEditingRef.current) return;
  ...
  map.on("mousemove", (event: MapMouseEvent) => {
    ...
    if (dragIndexRef.current === 0 || dragIndexRef.current === ring.length - 1) {
      ...
      ring[dragIndexRef.current] = coordinates;
    }
  ...
  map.on("click", (event: MapMouseEvent) => {
    ...
    if (isCreatingRef.current) {
      onDraftRingChange(appendPointToDraftRing(draftRingRef.current, point));
      return;
    }

    if (addPointModeRef.current && draftRingRef.current.length >= 4) {
      onDraftRingChange(
        insertPointIntoNearestSegment(draftRingRef.current, point)
      );
    }
  });
}