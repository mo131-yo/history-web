...
function createDraftPolygon(ring: Array<[number, number]>) {
  ...
  return {
    type: "FeatureCollection" as const,
    features: displayRing.length >= 4
      ? [{ type: "Feature" as const, geometry: { type: "Polygon" as const, coordinates: [displayRing] }, properties: {} }]
      : [],
  };
}
...
export default function HistoricalMap({
  ...
  useEffect(() => {
    ...
    addPointModeRef.current = addPointMode;
    draftRingRef.current = draftRing;
  }, [addPointMode, collection, draftRing, isCreating, isEditing, selectedSlug]);
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      ...
      map.addLayer({ id: "states-fill", type: "fill", source: "atlas-states", paint: {
        "fill-color": ["coalesce", ["get", "color"], "#c9a45d"],
        "fill-opacity": ["case", ["==", ["get", "slug"], selectedSlugRef.current ?? ""], 0.42, 0.2],
      }});
      map.addLayer({ id: "states-outline", type: "line", source: "atlas-states", paint: {
        "line-color": "#0f172a", "line-width": 1.2, "line-opacity": 0.95,
      }});
      map.addLayer({ id: "states-labels", type: "symbol", source: "atlas-states", layout: {
        "text-field": ["get", "name"],
        "text-font": ["Open Sans Semibold"],
      ...
      map.addLayer({ id: "states-selected-outline", type: "line", source: "atlas-states",
        filter: ["==", ["get", "slug"], ""],
        paint: { "line-color": "#f59e0b", "line-width": 3.5, "line-opacity": 1 },
      });
      map.addLayer({ id: "draft-fill", type: "fill", source: "draft-polygon",
        paint: { "fill-color": "#38bdf8", "fill-opacity": 0.12 },
      });
      map.addLayer({ id: "draft-outline", type: "line", source: "draft-polygon",
        paint: { "line-color": "#38bdf8", "line-width": 3, "line-dasharray": [1, 1] },
      });
      map.addLayer({ id: "draft-vertices", type: "circle", source: "draft-vertices",
        paint: {
          "circle-radius": ["interpolate", ["linear"], ["zoom"], 2, 5, 7, 8],
          "circle-color": "#f8fafc",
          "circle-stroke-color": "#0ea5e9",
      ...
      map.addLayer({ id: "states-hover-outline", type: "line", source: "atlas-states",
        filter: ["==", ["get", "slug"], ""],
      ...
      map.on("click", interactiveStateLayers, (event) => {
        ...
        if (bounds) {
          map.fitBounds(bounds, { padding: focusPaddingRef.current, maxZoom: 4.2, duration: 900, essential: true });
        } else if (center) {
          map.flyTo({ center, zoom: Math.max(Math.min(map.getZoom() + 0.4, 4), 3.2), speed: 0.8, curve: 1.2, essential: true });
        }
      });
  ...
  useEffect(() => {
    ...
    statesSource.setData(collection ?? { type: "FeatureCollection", features: [] });
    if (selectedSlug) {
      const selectedFeature = collection?.features.find((f) => f.properties.slug === selectedSlug);
      const bounds = getFeatureBounds(selectedFeature);
      const center = normalizeLngLatLike(selectedFeature?.properties.center) || getFeatureCenter(selectedFeature);
      if (bounds) {
        map.fitBounds(bounds, { padding: focusPaddingRef.current, maxZoom: 4, duration: 700, essential: true });
      } else if (center) {
        map.flyTo({ center, zoom: Math.max(Math.min(map.getZoom(), 3.8), 3), speed: 0.7 });
      }
    }
  ...
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !mapReadyRef.current || !map.isStyleLoaded() || !map.getLayer("states-selected-outline")) return;
    map.setFilter("states-selected-outline", ["==", ["get", "slug"], selectedSlug ?? ""]);
  }, [selectedSlug]);
