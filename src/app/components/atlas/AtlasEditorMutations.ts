...
export function validateEditorPayload(
  ...
  if (payload.name.length < 2)
    return { error: 'Нэр дор хаяж 2 тэмдэгт байна.' };
  if (payload.summary.length < 8)
    return { error: 'Тайлбар дор хаяж 8 тэмдэгт байна.' };
  return { payload };
}
...
export async function saveAtlasFeature(args: {
  ...
  const payload = {
    ...
    metadata: args.isCreating
      ...
      : {
          ...(args.selectedFeature?.properties.metadata ?? {}),
          periodName: args.validated.periodName,
        },
    coordinates: args.validated.coordinates,
  };
  const url = args.isCreating
    ? '/api/atlas/states'
    : `/api/atlas/states/${args.selectedFeature?.properties.slug}`;
    ...
    throw new Error(
      body?.details?.fieldErrors
        ? Object.entries(body.details.fieldErrors)
            ...
            .join(' | ')
        : (body?.error ?? (args.isCreating ? 'create-failed' : 'save-failed')),
    );
  }
...
export async function deleteAtlasFeature(year: number, slug: string) {
  const response = await fetch(`/api/atlas/states/${slug}?year=${year}`, {
    method: 'DELETE',
  });
