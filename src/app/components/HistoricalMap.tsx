...
const HistoricalMap = forwardRef<MapHandle, Props>(
  ({ year, isWhatIf, onSelectFeature }, ref) => {
    ...
    const loadData = useCallback(
      (m: maplibregl.Map, currentYear: number, alternate: boolean) => {
        const src = m.getSource('historical-data') as maplibregl.GeoJSONSource;
        if (!src) return;

        const dataPath =
          alternate && currentYear === 1206
