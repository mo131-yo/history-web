...
export const Sidebar = ({
  ...
  return (
    <aside
      className="relative flex h-full flex-col transition-all duration-500 border-r border-white/10 w-[400px] z-20 shadow-2xl"
      style={{
        background: isWhatIf
          ? 'linear-gradient(180deg, #1a0a0a 0%, #050505 100%)'
          : 'linear-gradient(180deg, #0a0c14 0%, #020308 100%)',
    ...
    >
      <div className="px-6 pt-8 pb-4 space-y-4">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <h1 className="text-4xl font-black tracking-tighter text-[#C5A059] font-serif">
              {year} <span className="text-lg font-normal opacity-50">он</span>
            </h1>
            {isWhatIf && (
          ...
          <button
            ...
            className="p-2 transition-all rounded-full hover:bg-white/5 text-white/20 hover:text-white"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <button
          onClick={onStartQuiz}
          className="group relative w-full overflow-hidden flex items-center justify-center gap-3 bg-gradient-to-r from-indigo-600 to-blue-700 hover:from-indigo-500 hover:to-blue-600 text-white py-3.5 rounded-xl transition-all font-bold text-[11px] uppercase tracking-[0.15em] shadow-lg shadow-indigo-500/20"
        >
          <BrainCircuit
            size={16}
        ...
        {!selectedFeature && (
          <div className="relative group">
            <Search
              className="absolute left-3.5 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-[#C5A059] transition-colors"
              size={15}
            />
            <input
              type="text"
              placeholder="Эзэнт гүрэн, тулаан хайх..."
              value={searchQuery}
      ...
      <div className="flex-1 px-6 py-4 overflow-y-auto custom-scrollbar">
        {selectedFeature ? (
          <div className="space-y-6 duration-500 animate-in fade-in slide-in-from-right-8">
            <div className="space-y-4">
              <button
                ...
                className="flex items-center gap-2 text-[10px] text-white/30 hover:text-[#C5A059] uppercase tracking-[0.2em] transition-all"
              >
              ...
              <div className="relative p-6 rounded-2xl border border-white/10 bg-white/[0.02] overflow-hidden">
                ...
                <h2
                  className="mb-3 font-serif text-3xl font-bold leading-tight"
              ...
              {activeTab === 'info' && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                  ...
                  <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5 space-y-3">
                    <h4 className="text-[10px] text-white/40 uppercase font-bold flex items-center gap-2">
              ...
              {activeTab === 'military' && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-4 border rounded-xl bg-red-500/5 border-red-500/10">
                      <p className="text-[9px] text-red-500/50 uppercase font-bold mb-1">
                        Армийн тоо
                      </p>
                    ...
                    <div className="p-4 border rounded-xl bg-orange-500/5 border-orange-500/10">
                      <p className="text-[9px] text-orange-500/50 uppercase font-bold mb-1">
                        Гол нэгж
                      </p>
              ...
              {activeTab === 'stats' && (
                <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2">
                  {[
                    ...
                    <div
                      key={idx}
                      className="flex items-center justify-between p-3.5 rounded-xl bg-white/[0.01] border border-white/5 hover:bg-white/[0.03] transition-colors"
                    >
          ...
          <div className="space-y-4">
            <h3 className="text-[10px] text-white/20 uppercase font-bold tracking-[0.2em] pl-2">
              Бүс нутгийн эзэнт гүрнүүд
            </h3>
            {searchResults.length === 0 ? (
              ...
              <div className="grid gap-3">
                {searchResults.map((f, i) => (
                  ...
                  >
                    <div
                      className="absolute top-0 bottom-0 left-0 w-1 transition-all group-hover:w-2"
                      style={{ background: f.properties.color || '#C5A059' }}
                    />
                    <div className="flex items-center justify-between">
                      <div className="space-y-1">
                        <h4 className="text-base font-bold text-white/90 font-serif group-hover:text-[#C5A059] transition-colors">
                          {f.properties.name}
                        </h4>
                        <div className="flex items-center gap-3 opacity-40">
                          <span className="text-[10px] flex items-center gap-1 uppercase">
                            <Users size={10} /> {f.properties.leader}
                          </span>
                          <span className="text-[10px] flex items-center gap-1 uppercase">
                            <Landmark size={10} /> {f.properties.capital}
                          </span>
                        </div>
                      </div>
                      <ChevronRight
                        size={16}
                        className="transition-all transform text-white/10 group-hover:text-white group-hover:translate-x-1"
                      />
                    </div>
                  </button>
                ))}
      ...
      <div className="flex items-center justify-between px-6 py-4 border-t bg-black/40 border-white/5">
        ...
        <span className="text-[9px] text-white/20 font-mono tracking-tighter">
          DATA_FETCH_OK: 1206_HISTORICAL_CORE
        </span>
      </div>
    </aside>
  );
